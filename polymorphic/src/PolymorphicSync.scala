...
object PolymorphicSync extends IOApp.Simple:
  ...
  val aDelayedIO: IO[Int] = IO.delay: // "suspend" computations in IO
  ...
  val syncIO: Async[IO] = Sync[IO] // given Sync[IO] in scope
  ...
  import cats.syntax.functor.toFunctorOps // map extension method
  object Console:
    def make[F[_]](using sync: Sync[F]): F[Console[F]] =
      sync.pure((System.in, System.out)).map { (in, out) =>
        new Console[F]:
          def println[A](a: A): F[Unit] =
