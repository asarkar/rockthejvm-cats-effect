...
object PolymorphicAsync extends IOApp.Simple:
  ...
  val asyncIO: Async[IO] = Async[IO] // given/implicit Async[IO]
  ...
  val myExecutionContext: ExecutionContextExecutorService =
    ExecutionContext.fromExecutorService(threadPool)
  val asyncMeaningOfLife_v3: IO[Int] =
    asyncIO.evalOn(IO(42).debug(), myExecutionContext).guarantee(IO(threadPool.shutdown()))

  // never
  val neverIO = asyncIO.never

  /** Exercises 1 - implement never and async_ in terms of the big async. 2 - tuple two effects with
  ...
  import cats.syntax.functor.toFunctorOps // flatMap extension method
